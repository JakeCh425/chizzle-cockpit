...
// ─── LiveStamp — isolated 1s clock so the parent grid doesn't re-render ─────
const LiveStamp = memo(function LiveStamp() {
  ...
  return (
    <div
      className="flex items-center justify-end gap-1.5 mt-2 text-[10px] uppercase tracking-wider text-slate-gray"
      data-testid="text-last-updated"
      aria-live="polite"
      aria-atomic="true"
    >
      <span
        className="inline-block w-1.5 h-1.5 rounded-full bg-signal-green shadow-[0_0_6px_hsl(var(--signal-green)/0.7)] animate-pulse"
        aria-hidden="true"
      />
      <span>Updated {stamp} CT · Live</span>
    </div>
  );
});

const DEFAULT_FALLBACK = ["SMH", "QQQ", "SPY"];
...
export default function MiniChartGrid({
  ...
  const openFullChart = useCallback(
    (symbol: string, interval: Interval) => setModal({ symbol, interval }),
    []
  );
